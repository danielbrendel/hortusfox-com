...
/*
    Asatru PHP - Migration for AffiliateModel
*/

/**
 * This class specifies a migration
 */
class AffiliateModel_Migration {
    ...
    public function up()
    {
        $this->database = new Asatru\Database\Migration('AffiliateModel', $this->connection);
        $this->database->drop();
        $this->database->add('id INT NOT NULL AUTO_INCREMENT PRIMARY KEY');
        $this->database->add('label VARCHAR(512) NOT NULL');
        $this->database->add('description VARCHAR(1024) NOT NULL');
        $this->database->add('logo VARCHAR(512) NOT NULL');
